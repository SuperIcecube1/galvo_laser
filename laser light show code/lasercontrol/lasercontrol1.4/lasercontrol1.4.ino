...
const int RED_LASER_PIN = 18;
const int GREEN_LASER_PIN = 21;
const int BLUE_LASER_PIN = 19;
...
#define EEPROM_BEAT_DROP_EFFECT_ADDR 336 // Adjusted address
...
// Beat drop effect
int beatDropEffect = 0; // Initialize to 0 (no effect)
...
bool beatDropActive = false;

// IR receiver
IRrecv irrecv(IR_RECEIVER_PIN);

// Queue to send effect numbers to the effect task
...
// Fetch data interval
const unsigned long fetchInterval = 500; // Fetch data every 500ms
...
void setup() {
  ...
  Serial.print("Beat Drop Effect: ");
  Serial.println(beatDropEffect);

  // Initialize the effect queue
  effectQueue = xQueueCreate(10, sizeof(int)); // Queue size 10, each item is an int
  if (effectQueue == NULL) {
    Serial.println("Failed to create effectQueue!");
  }

  // Create the EffectTask on Core 1
...
void EffectTask(void *pvParameters) {
  ...
  for (;;) {
    // Wait indefinitely for an effect number to be available
    if (xQueueReceive(effectQueue, &effectNumber, portMAX_DELAY)) {
      switch (effectNumber) {
        case 1:
          effect1();
          break;
...
// Function to run the specified effect by sending it to the EffectTask via the queue
void runEffect(int effectNumber) {
  serialPrintln("Running effect " + String(effectNumber));

  // Send the effect number to the queue
  if (xQueueSend(effectQueue, &effectNumber, portMAX_DELAY) != pdTRUE) {
    serialPrintln("Failed to send effect to the queue");
  }
}
...
void startMainMode() {
  ...
  server.on("/set_beat_drop_effect", HTTP_POST, handleSetBeatDropEffect);
  server.begin();
...
void handleRootMain() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j <= 20; j++) { // Now supports up to 20 effects
  ...
  // Form to set Beat Drop Effect
  html += "<form action='/set_beat_drop_effect' method='post'>";
  html += "<h2>Set Beat Drop Effect</h2>";
  html += "Beat Drop Effect: <select name='beat_drop'>";
  for (int j = 0; j <= 20; j++) { // Now supports up to 20 effects
    html += "<option value='" + String(j) + "'";
    if (beatDropEffect == j) {
      html += " selected";
...
// Handle setting Beat Drop Effect
void handleSetBeatDropEffect() {
  if (server.hasArg("beat_drop")) {
    int value = server.arg("beat_drop").toInt();
    beatDropEffect = value;
    saveSettings();
    server.send(200, "text/html", "<html><body><h1>Beat Drop Effect Saved</h1><a href='/'>Back</a></body></html>");
  } else {
    server.send(400, "text/html", "<html><body><h1>Bad Request</h1></body></html>");
  }
}
...
void saveSettings() {
  ...
  // Store Beat Drop Effect
  EEPROM.put(EEPROM_BEAT_DROP_EFFECT_ADDR, beatDropEffect);
...
void loadSettings() {
  ...
  // Read Beat Drop Effect
  EEPROM.get(EEPROM_BEAT_DROP_EFFECT_ADDR, beatDropEffect);
...
void handleEffects(int is_beat_drop, int energy_level) {
  if (is_beat_drop == 1) {
    if (!beatDropActive) {
      // Start beat drop effect
      beatDropActive = true;
      runEffect(beatDropEffect);
      // After effect finishes
      beatDropActive = false;
    }
  } else {
    // Run energy level effect
    if (energy_level >= 0 && energy_level <= 3) {
      ...
      // Invalid energy level, turn off lasers
      analogWrite(RED_LASER_PIN, 0);
      analogWrite(GREEN_LASER_PIN, 0);
      analogWrite(BLUE_LASER_PIN, 0);
    }
